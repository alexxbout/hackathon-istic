    ...
    <div class="h-screen w-screen flex flex-col">
        ...
        <div class="flex items-center justify-center w-full h-full">
            <div class="flex flex-col items-center w-full h-full py-10 gap-y-5 mx-80">
                ...
                <!-- Searchbar -->
                <SearchBar ref="searchBarElement" :experienceOptions="experienceOptions" :role="role" :skillOptions="skillOptions" @updateExperience="updateExperience" @updatePeriod="updatePeriod" @updateSkills="updateSkills" />

                <div v-if="hotreload" class="grid grid-cols-2 grid-rows-3 gap-2 content-between w-full">
                    <Card v-for="profil in paginatedCustomProfils" :id="profil.id!" :key="profil.id" :experience="profil.experience" :isChecked="selectedProfiles.has(profil.id!)" :nom="profil.nom" :photo="profil.profile_picture" :prenom="profil.prenom" :role="role" @click="toggleSelection(profil.id!)" @@click="handleShowDetails(profil.id!)" />
                </div>
...
// Options des filtres
const experienceOptions = ref<{ value: number; label: string }[]>(
    profils
        .map((profil) => ({
